...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    if (name && nickname && mood) {
      const parsed = { name, nickname, mood, color }
      setForm(parsed)
      setPoem(generatePoem(parsed))
    }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[radial-gradient(circle_at_top_center,#1a103c_0%,#0a0e17_100%)] px-6">
      ...
      {!isSharedView && (
        <div className="mt-10 space-y-5 w-full max-w-md">
          ...
          <button onClick={downloadCard} className="w-full py-4 rounded-2xl bg-white/10 border border-white/20 text-white flex items-center justify-center gap-3 hover:scale-105 transition">
            <FaDownload /> Download Card
          </button>

          <button onClick={handleRewrite} className="w-full py-4 rounded-2xl bg-linear-to-r from-purple-600 to-pink-600 text-white flex items-center justify-center gap-3 hover:scale-105 transition">
            <FaRedo /> Write Another
          </button>

          <button onClick={() => setShowShare(!showShare)} className="w-full py-4 rounded-2xl bg-pink-500 text-white flex items-center justify-center gap-3 hover:scale-105 transition">
            <FaLink /> Share With Crush
          </button>

          <Link href="/">
            <button className="w-full py-4 rounded-2xl bg-white/5 border border-purple-400/40 text-purple-200 flex items-center justify-center gap-3 hover:scale-105 transition">
              <FaHome /> Back to Home
            </button>
          </Link>
        </div>
